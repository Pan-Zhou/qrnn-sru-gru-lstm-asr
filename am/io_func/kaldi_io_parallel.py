...
class KaldiDataReadParallel(object):
    ...
    def __init__(self, scp_file , read_opts = None):

        self.scp_file = scp_file     # path to the .scp file
...
#        self.cur_frame_num = 0
        self.alignment = {}
        if read_opts.has_key('num_streams'):
            self.num_streams = read_opts['num_streams']
        else:
            self.num_streams = 1
    ...
    def load_next_nstreams(self):
        ...
        while nstreams < self.num_streams:
            ...
            if True: #(len(ali_utt) * 2 + 1) < len(utt_mat):
                ...
                feat_mat.append(skip_frame(make_context(utt_mat, self.read_opts['lcxt'], self.read_opts['rcxt']),self.read_opts['skip_frame']))
