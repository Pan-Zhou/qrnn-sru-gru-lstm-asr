...
class Model(nn.Module):
    def __init__(self, words, args):
        ...
        if args.gru:
            self.rnn = nn.GRU(self.n_d,self.n_d,sef.depth,dropout=args.rnn_dropout)
        if args.sru:
        ...
        if args.qrnn:
###need to complete QRNN
            self.rnn = MF.QRNN(self.n_d,sef,n_d,self.depth,dropout=args.rnn_dropout,rnn_dropout =args.rnn_dropout,
                    use_tanh = 1) 
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--gru", action="store_true")
    argparser.add_argument("--qrnn", action="store_true")
    argparser.add_argument("--sru", action="store_true")
